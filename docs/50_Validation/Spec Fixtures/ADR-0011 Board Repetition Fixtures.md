...
- KO-01／02／05／06はP0から正規Application commandを実行し、battle-local historyを再構築するtrue replay。KO-06はsourceの`stone_kind=blood`も保持し、generic stone commandへの正規化を明示する。
...
- KO-06のexact `stone_kind=blood` assertionはDomain evidenceで維持し、Application replayでは同じ到達可能command sequenceとgeneric stone commandへのmetadata normalizationを併記する。
