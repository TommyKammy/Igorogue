...
2026-07-11 — occupied gateとTASK-0005の仮配置・同時capture後に、明示的`Normal`／`TerminalCapture` access、capture後exact effective-liberty snapshotによる自殺手、全戦闘履歴による反復の順で判定する`PlacementLegalityEvaluator`を実装。terminal grantだけでは合法化せず、即時相手group captureをDomainで確認し、自殺手・反復・王石capture例外を作らない。illegal評価はaccepted candidateを返さず、合法評価だけをexact candidate／評価時historyへbindする。
...
2026-07-11 — post-commit API／determinism reviewで、任意の未出現keyやstale／cross-candidate評価をhistoryへ登録できるMEDIUM findingを検出。raw key登録を廃止し、合法評価がexact candidateとexact historyを保持し、source/current/key/Seenを再検証してboard、ordered facts、next historyを一体で返す`LegalPlacementCommit`へ修正した。illegal、stale、同値別history、cross-candidate、arbitrary-key API不存在を追加test化し、API、determinism、独立scopeの3再reviewすべてでfinding解消と`APPROVE`を確認。
...
2026-07-11 — GitHub Actions run `29138193327`でGovernance、Pure .NET build／128 tests／simulator smoke、Godot .NET headless smoke／Windows debug exportの全3 job成功を確認。人間判断でPR #7をmergeし、merge commit `307998c84f010f8a41c56e8c19fd5aac53aef522`を確認して`done`へ遷移。
...
- `tools/dev/test` — exit 0。exact .NET SDK `8.0.422`、locked restore、Release build、warning 0／error 0。Domain 111、Application 12、Architecture 5、合計128 testが成功。
...
- GitHub Actions run `29138193327` — PR #7 head `aafcde8473d647ca18e29ba27c66206d6e2f9973`で全3 job成功。人間判断でPR #7をmergeし、merge commit `307998c84f010f8a41c56e8c19fd5aac53aef522`を確認。
...
timed／continuous stateからexact `EffectiveLibertySnapshot`を生成するcalculatorは、そのruntime effect導入taskへ延期する。mandatory失効captureの重複topology観測、王石勝敗、golden replay、full BattleState checksum／replay統合はそれぞれ後続の仕様task／TASK-0007／TASK-0009／TASK-0010〜0011で統合する。
