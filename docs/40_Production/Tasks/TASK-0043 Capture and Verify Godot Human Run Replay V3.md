...
本実装のmacOS headless win／loss／save-race evidence、Windows debug export、PR #35 CIは成功したが、macOS graphicalで人間が操作したloss／win terminal artifactとterminal overlayのhuman visual確認は未実施である。Windows上のruntime save／renameも未実行であり、Windows export successをruntime filesystem evidenceとして扱わない。本TASKは`review`に留め、graphical human UATとhuman merge後のcloseoutより前に`done`を主張しない。
