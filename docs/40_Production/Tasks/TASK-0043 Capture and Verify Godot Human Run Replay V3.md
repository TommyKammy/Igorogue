...
旧graphical UAT process PID `37746`を外部からdefault `kill`で終了した直後、macOSがGodot 4.7 .NETの`EXC_CRASH (SIGABRT)`を報告した。crash stackはCoreCLR `TerminationRequestHandlingRoutine`／`ExitProcess`とGodot native destructorのmutex競合だけで、Igorogue C#、game command、Replay serializer frameを含まない。terminal前でartifactも未生成だったため、これはIgorogue product failureではなくhost-toolchain shutdown raceと分類する。graphical UAT processはwindow close／`Cmd+Q`で正常終了を待ち、default `kill`を通常cleanupへ使用しない。
...
2026-07-18 — macOS crash reportのGodot PID `37746`、launch `09:27:38`、crash `11:04:02`を、同PIDへ行ったexternal terminationと照合した。crashed threadはCoreCLR Unix terminationから`exit`／Godot native destructor／`std::mutex::lock`／`abort`へ到達し、main AppKit threadもnative `recursive_mutex::lock`でabortした。19秒後に起動したmerged-main replacement processは同じGodot 4.7 .NETで正常稼働し、post-merge automated evidenceもgreen。product code change不要と判定し、GUI cleanupをgraceful quitへ修正した。
...
- macOS Godot shutdown crash triage — PID／timestamp correlation、native CoreCLR／Godot teardown stack、Igorogue frame不在、artifact未生成、healthy replacement processを確認。classification `host-toolchain-shutdown`、product code change required `false`。
