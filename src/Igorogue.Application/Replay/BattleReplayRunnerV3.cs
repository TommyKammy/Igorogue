...
public static class BattleReplayRunnerV3
{
    public static CoreDuelBattleReplayResult Replay(
        ...
        CoreDuelBattleSession initialSession)
    {
        ...
        var results = new List<CoreDuelBattleCommandResult>(document.Attempts.Count);
        foreach (var attempt in document.Attempts)
        {
            ...
            var command = BattleReplayCommandCodecV3.Decode(
                attempt,
                session.State.BattleState.Board.Geometry);
        ...
        return new CoreDuelBattleReplayResult(session, results);
    }
}

public sealed class CoreDuelBattleReplayResult
{
    private readonly CoreDuelBattleCommandResult[] commandResults;
    private readonly ReadOnlyCollection<CoreDuelBattleCommandResult> commandResultView;

    internal CoreDuelBattleReplayResult(
    ...
    public CoreDuelBattleSession FinalSession { get; }

    public IReadOnlyList<CoreDuelBattleCommandResult> CommandResults => commandResultView;
}
